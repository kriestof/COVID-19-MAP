...
export default function downloadChartPng(MARGIN) {
  ...
  downloadSvg.attr("width", chartSvg.node().width.baseVal.value*SCALE)
  downloadSvg.attr("height", (chartSvg.node().height.baseVal.value + 100)*SCALE)

  downloadSvg.selectAll(".remove-label").remove()
  downloadSvg.selectAll(".text-label").attr("x", 40)

  downloadSvg.selectAll(".y-axis, .x-axis, .country-lines, .legend").attr("transform", `translate(0, ${50*SCALE}) scale(${SCALE})`)

  let downloadSvgG = downloadSvg.append("g").attr("transform", `scale(${SCALE})`)
  ...
  downloadSvgG
      ...
      .attr("transform", "translate(20, 15) scale(0.15)")

  downloadSvgG
    .append("text")
    .text(`source: covid19map.icm.edu.pl | formula:  ${d3.select("#formula").node().value}`)
    .attr("y", downloadSvg.attr("height")/SCALE-47).attr("x", MARGIN.x)
    .attr("fill", "#595959")
    .style("font-family", "sans-serif")
    ...
    downloadSvgG
      .append("text")
      .text("Based on data collected by Johns Hopkins CSSE and World Bank.")
      .attr("y", downloadSvg.attr("height")/SCALE-7).attr("x", MARGIN.x)
      .attr("fill", "#595959")
      .style("font-family", "sans-serif")
      .style("font-size", "12px")
